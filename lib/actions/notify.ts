...
// Escape any user-supplied text before it lands in the email HTML body.
// Email clients vary widely in how strictly they sanitize — a crafted
// userName or companyName like `"><img src=x onerror=...>` would render
// in some webmail clients. Apply to every interpolation that traces back
// to user input or DB strings that themselves came from users.
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; background: #F6F7F9; padding: 40px; border-radius: 12px; max-width: 520px; margin: 0 auto;">
        <div style="background: white; border-radius: 10px; padding: 32px; border: 1px solid #E5E7EB;">
          ...
          <p style="color: #64748B; margin: 0 0 24px; font-size: 14px;">${esc(now)} WIB</p>

          <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
            <tr>
              <td style="padding: 8px 0; color: #64748B; font-size: 13px; width: 80px;">Email</td>
              <td style="padding: 8px 0; color: #0F172A; font-size: 13px; font-weight: 600;">${esc(userEmail)}</td>
            </tr>
            ${userName ? `<tr>
              <td style="padding: 8px 0; color: #64748B; font-size: 13px;">Nama</td>
              <td style="padding: 8px 0; color: #0F172A; font-size: 13px;">${esc(userName)}</td>
            </tr>` : ''}
          </table>
      ...
      <div style="font-family: system-ui, sans-serif; background: #F6F7F9; padding: 40px; border-radius: 12px; max-width: 520px; margin: 0 auto;">
        <div style="background: white; border-radius: 10px; padding: 32px; border: 1px solid #E5E7EB;">
          ...
          <p style="color: #334155; font-size: 14px; margin: 0 0 24px;">
            Akun Anda telah disetujui sebagai <strong>${esc(role === 'accountant' ? 'Akuntan' : 'Staff')}</strong>.
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: monospace; background: #0A0A0C; color: #E4E4E7; padding: 32px; border-radius: 12px;">
        ...
        ${reason ? `<p>Alasan: <strong>${esc(reason)}</strong></p>` : ''}
      ...
      <div style="font-family: system-ui, sans-serif; background: #F6F7F9; padding: 40px; border-radius: 12px; max-width: 520px; margin: 0 auto;">
        <div style="background: white; border-radius: 10px; padding: 32px; border: 1px solid #E5E7EB;">
          ...
          <p style="color: #334155; font-size: 14px; margin: 0 0 8px;">
            Payroll <strong>${esc(companyName)}</strong> periode <strong>${esc(BULAN[bulan])} ${tahun}</strong> telah dikunci.
          </p>
          <p style="color: #64748B; font-size: 13px; margin: 0 0 24px;">Dikunci oleh: ${esc(lockedBy)}</p>
          <a href="${appUrl}/dashboard"
            style="display: inline-block; background: #2563EB; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">
            Lihat Dashboard →
          </a>
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: monospace; background: #0A0A0C; color: #E4E4E7; padding: 32px; border-radius: 12px;">
        ...
        <p>Import <strong>${esc(companyName)}</strong> — ${esc(BULAN[bulan])} ${tahun}</p>
